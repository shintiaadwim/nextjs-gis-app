...
const dropdownItemClassName = 'flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-100'
...
export function DropdownMenu({ children, className, anchor, ...props }: DropdownMenuProps) {
    ...
    return (
        <div
            className={clsx(
                'invisible absolute z-50 rounded-xl border border-zinc-200 bg-white p-1 opacity-0 shadow-lg transition group-focus-within/dropdown:visible group-focus-within/dropdown:opacity-100 group-hover/dropdown:visible group-hover/dropdown:opacity-100',
                positionClassName,
                className,
            )}
            {...props}
